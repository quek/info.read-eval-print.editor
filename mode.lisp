...
(defgeneric get-key (key-map editor-mode keyseq)
  (:method (key-map editor-mode keyseq)
    (apply #'values
           (or (ensure-list (gethash keyseq (key-map-table key-map editor-mode)))
               (collect-first
                (mapping ((inherit (scan (inherit-of key-map))))
                  (multiple-value-list (get-key inherit
                                                editor-mode
                                                keyseq))))))))

(defmethod get-key :around (key-map editor-mode keyseq)
  (apply #'values (aprog1 (multiple-value-list (call-next-method))
                    (when (and (temp-key-map-of key-map) it)
                      (pop (temp-key-map-of key-map))))))

(defgeneric set-key (key-map editor-mode keyseq command)
  (:method (key-map editor-mode keyseq command)
    (setf (gethash keyseq (key-map-table key-map editor-mode))
          command)))

(defmethod set-key :around (key-map editor-mode keyseq command)
...
(defgeneric push-temp-key-map (key-map temp-key-map)
  (:method (key-map temp-key-map)
    (push (temp-key-map-of key-map) temp-key-map)))
...
(defvar *g-key-map* (make-instance 'key-map))
(loop for (keyseq command)
